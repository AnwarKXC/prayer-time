import { _ as _export_sfc, u as useI18n, e as useFetch, a as __nuxt_component_0$2, b as __nuxt_component_0$3 } from './server.mjs';
import { useSSRContext, withAsyncContext, mergeProps, unref, withCtx, createTextVNode, toDisplayString, createVNode } from 'vue';
...
const _sfc_main$6 = {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-[244px] bg-white rounded-xl border border-gray-200 main__banner" }, _attrs))}><div class="cent h-full w-full"><div class="relative md:grid grid-cols-4 p-4 gap-1 w-full"><div class="md:col-span-3 flex items-center size-full"><div class="grid gap-4"><div class="text-zinc-700 md:text-[15px] text-[13px] font-normal font-[&#39;Almarai&#39;]">${ssrInterpolate(unref(activeOne).sub_title)}</div><h2 class="h__primary max-w-[600px] max-md:max-w-[500px] leading-10">${ssrInterpolate(unref(activeOne).title)}</h2><div class="flex">`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            return [
              createTextVNode(toDisplayString(unref(links).title_download), 1)
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</div></div></div><div class="flex md:justify-center justify-end items-center max-md:absolute rtl:left-0 ltr:right-0 bottom-0 max-md:size-[120px] max-xs:size-[90px] size-full ltr:scale-x-[-1] transform max-md:mx-4 md:w-full"><img${ssrRenderAttr("src", unref(domain) + unref(activeOne).image.data.attributes.url)} alt="" class="rounded w-full"></div></div></div></div>`);
    };
  }
};
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(countries).data, (res) => {
        var _a;
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/app/" + res.attributes.api_country_code + "/" + res.id + "/" + ((_a = res.attributes.default_prayer_time_city.data) == null ? void 0 : _a.attributes.api_city_code),
          key: res.id
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
