    ...
    <div class="content-widget mx-sm-0 rounded-sm-0">
        <div class="row p-4">
            <div class="col-lg-12 px-0">
                <h3 class="text-theme mb-0 rtl fw-700">{{ $t('moreplaybanner.heading') }}</h3>
            </div>
            <div class="col-lg-12 px-0">
                <div class="MoreSuratBox mt-3 mb-2">
                    <div class="MoreSuratList" v-for="play in playlist">
                        <router-link :to="'/prayer-now-playlist/' + play.playlist_id + '/' + this.langCode"
                            class="MoreSuratList-content text-decoration-none w-100 justify-content-end">
                            <div>
                                <h5>{{ play.name }}</h5>
                                <p>{{ play.description }}</p>
                            </div>
                            <div class="more_play_list">
                                <AppImage :imageSrc="MorePlayListImg + play.image" />
                            </div>
                        </router-link>
                        <div class="MoreSuratList-play">
                            <p class="mb-0">{{ play.localize_tracks?.length }} تشغيل</p>
...
export default {
    data() {
        return {
            MorePlayListImg: '/playlistImages/',
            playlist: [],
    ...
    methods: {
        async fetchMorePlaylist() {
            try {
                ...
                const response = await axios.get(`${apiUrl}/api/get-more-playlist-api/${e}`);
                if (response.status == 200) {
                    this.playlist = response.data.playlist;
                    // console.log('resd :', response.data.playlist);
                }
            } catch (error) {
                // Handle errors
